...
export default function UploadForm({ onFileSelect, onUpload, onRandom, uploading, file, result }: Props) {
	...
	return (
		<Card className="max-w-4xl mx-auto p-8 bg-white backdrop-blur-lg border border-white/20 rounded-2xl shadow-lg mt-5">
			<CardHeader className="text-center">
				...
				<p className="text-gray-600">Upload a scenic photo or let AI surprise you ✨</p>
			</CardHeader>
			...
			<div
				...
				className={`relative cursor-pointer border-2 border-dashed rounded-lg p-6 max-w-3xl flex flex-col items-center justify-center text-gray-400 transition
          			${dragOver ? "border-pink-500 bg-pink-50/20 text-pink-400" : "border-gray-400 bg-transparent"}`}
			>
				{!file ? (
					<>
						<Input
					...
					<>
						...
						{result && (
							<p className="mx-auto mt-2 text-gray-700 dark:text-gray-300">
								<strong>Detected Scene: </strong> {result.scene_type.toUpperCase()}
							</p>
						)}
					</>
				)}
			...
			<div className="mt-6 flex justify-center gap-4">
				...
				<Button variant="outline" onClick={onRandom} disabled={uploading} className="flex items-center gap-2">
					<Sparkles />
					Suggest Random
				</Button>
			</div>
