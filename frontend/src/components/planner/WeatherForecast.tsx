...
function getIcon(description: string) {
	...
	if (desc.includes("thunder")) return <Zap className="w-5 h-5 text-yellow-400" />
	if (desc.includes("rain")) return <CloudRain className="w-5 h-5 text-blue-500" />
	if (desc.includes("snow")) return <Snowflake className="w-5 h-5 text-cyan-300" />
	if (desc.includes("cloud")) return <Cloud className="w-5 h-5 text-gray-600" />
			...
			<CardContent>
				...
				>
					{forecast.map((day, index) => {
						...
						return (
							<div key={index} className="flex flex-col items-center text-xs min-w-0 ">
								<div className="text-2xl">{emoji}</div>
								{/* {getIcon(day.description)} */}
								<p className="font-medium">{format(parseISO(day.date), "EEE")}</p>
								<p className={`capitalize font-semibold ${colorClass}`}>{day.description}</p>
								<p className="text-black font-semibold">
									{Math.round(day.temp)}°{unit === "metric" ? "C" : "F"}
								</p>
								<p className="text-blue-600 text-xs">{day.humidity}% humidity</p>
							</div>
						)
					})}
				</div>
